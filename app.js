...
function writeFile(_name, cont) {
  ...
  _writeFile(
    __dirname + `/outputSchemas/${name[0]}.ttl`,
    cont.shacl,
    (err, contents) => {}
  );
  ...
  _writeFile(
    __dirname + `/outputLog/${name[0]}_log.txt`,
    log,
    (err, contents) => {}
  );
...
function stats() {
  ...
  if (totalFiles < STOP + 1) {
    ...
    console.log(
      `Average time conversion: ${
        finalTimeConversion / (session - 1)
...
function readRepository() {
  ...
  readdir(__dirname + "/inputSchemas", (err, filenames) => {
    if (err) {
      return;
    }
    ...
    filenames.forEach((filename) => {
      ...
      readFile(
        __dirname + "/inputSchemas/" + filename,
        "utf8",
        (err, contents) => {
          try {
...
function readSingleFile(name) {
  t0 = performance.now();
  readFile(__dirname + "/inputSchemas/" + args[2], "utf8", (err, contents) => {
    try {
      var cont = JSON.parse(contents);
    } catch {}
