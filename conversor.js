import JS4Geo from "./JS4GeoDefs.js";
import { performance } from "perf_hooks";

export function check(schema, runs) {
  try {
...
export function start(schema) {
  var JS4GeoDataTypes = {
    ...
    multiLineString: "multiLineStringShape",
    multiPolygon: "multiPolygonShape",
    feature: "featureShape",
    featureCollection: "featureCollectionShape",
    geometryCollection: "geometryCollectionShape",
  };
  var dataTypes = {
    string: "string",
    integer: "integer",
  ...
  var prefix = {
    dash: "@prefix dash: <http://datashapes.org/dash#> .",
  ...
  function createSchemaSectionElementes(element) {
    ...
    nodesReady["JS_id"] =
      setMainNodeShape() +
      create_New_Complex_NodeShape_Structure(element, "JS_id");
  }
  ...
  function setListValidationArrayProperty(
    element,
    name,
    required,
    last,
    ...
    if ("$ref" in element.items) {
      ...
      if (typeItem in JS4GeoDataTypes) {
        ...
        local +=
          addSpaces() +
          `sh:datatype ex:${typeItem}Shape;\n` +
          addSpaces(-1) +
          "];\n";
      } else {
        local +=
          addSpaces() +
          `sh:datatype ex:${typeItem}_Shape;\n` +
          addSpaces(-1) +
          "];\n";
      }
      ...
      if (element.items.type in dataTypes) {
        ...
        local +=
          addSpaces() + `sh:datatype xsd:${dataTypes[element.items.type]};\n`;
  ...
  function setTupleArrayProperty(element, name, required, last) {
    ...
    element.items.forEach((element_) => {
      ...
      if (element_.type in dataTypes) {
        ...
        local +=
          addSpaces() +
          `sh:datatype xsd:${dataTypes[element_.type]};\n` +
          addSpaces(-1) +
          "];\n";
      } else {
        addElementsCount("elements", 1, name);
  ...
  function setPrimitiveProperty(element, name, required, last) {
    ...
    if (name != null) {
      local +=
        ...
        `sh:path ex:${name};\n` +
        addSpaces() +
        `sh:datatype xsd:${dataTypes[element.type]};\n`;
    } else {
      ...
      if (item in constraints) {
        if (item == "const" || item == "enum") {
          try {
            ...
            local +=
              addSpaces() + `sh:${constraints[item]} ("${element[item]}");\n`;
          }
        } else if (
          item == "pattern" ||
          item == "description" ||
          item == "title" ||
          (item == "default" && element.type == "string")
        ) {
          local +=
            addSpaces() + `sh:${constraints[item]} "${element[item]}";\n`;
        } else {
  ...
  function setShInProperty(element, name, item, required, last, specialCase) {
    ...
    if (name == "const" || name == "enum") {
      ...
      try {
        ...
        local +=
          addSpaces() + `sh:${constraints[name]} ("${element[name]}");\n`;
      }
    } else if (
      name == "pattern" ||
      name == "description" ||
      name == "title" ||
      (name == "default" && element.type == "string")
    ) {
      ...
      if ("$ref" in element_) {
        ...
        if (element_.type) {
          if (element_.type in dataTypes) {
            ...
            local += setPrimitiveProperty(
              element_,
              null,
              checkRequired(element_, i),
              false
            );
            ...
            if (element_[i].$ref) {
              ...
              if (typeItem in JS4GeoDataTypes) {
                ...
                local +=
                  ...
                  `sh:path ex:${i};\n` +
                  addSpaces() +
                  `sh:node ex:${typeItem}Shape;\n`;
              } else {
                local +=
                  addSpaces(1) +
                  `sh:path ex:${i};\n` +
                  addSpaces() +
                  `sh:node ex:${typeItem}_shape;\n`;
              }
  ...
  function create_New_Complex_NodeShape(element, name) {
    ...
    nodesReady[name] =
      node + create_New_Complex_NodeShape_Structure(element, name);
    scope = 1;
  }
    ...
    if (propertiesElements != undefined) {
      ...
      for (var item in propertiesElements) {
        if (propertiesElements[item].type in dataTypes) {
          local += setPrimitiveProperty(
            propertiesElements[item],
            item,
            checkRequired(element, item),
            checkLastElement(propertiesElements, item)
          );
          ...
          if (typeItem in JS4GeoDataTypes) {
            ...
            local += setComplexNodeShape(
              ...
              item,
              checkRequired(element, item),
              propertiesElements[item].$ref.split("/")[2],
              checkLastElement(propertiesElements, item)
            );
          }
        } else if (item in anotherConstraints) {
          local += setOthersProperty(
            propertiesElements[item],
            item,
            null,
            null,
          ...
          for (var i in propertiesElements[item]) {
            if (i in constraints) {
              local += setShInProperty(
                propertiesElements[item],
                i,
                item,
                checkRequired(element, item),
                checkLastElement(element, item),
                true
              );
      ...
      if (element.type == "array") {
        ...
        if ("allOf" in element) {
          ...
          local += setOthersProperty(
            ...
            null,
            null,
            checkLastElement(element, "allOf")
          );
          ...
          local += setOthersProperty(
            ...
            null,
            null,
            checkLastElement(element, "anyOf")
          );
          ...
          local += setOthersProperty(
            ...
            null,
            null,
            checkLastElement(element, "oneOf")
          );
        } else {
          local += ".\n";
        }
    ...
    if (name != null) {
      if (JS4GeoType != null) {
        local +=
          ...
          `sh:path ex:${name};\n` +
          addSpaces() +
          `sh:node ex:${JS4GeoDataTypes[JS4GeoType]};\n`;
      }
      if (required) {
        local += addSpaces() + `sh:minCount 1;\n`;
      }
  ...
  function create_New_JS4Geo_NodeShape(element, name) {
    ...
    if (name == "feature" || name == "featureCollection") {
      ...
      newNodes["properties"] =
        element.definitions.feature.properties.properties;
    }
  ...
  function checkUndefined(element, primitive) {
    for (var i in element) {
      if (
        !(i in constraints || i in anotherConstraints || i in jsonReservedWords)
      ) {
        var local = `{"${i}": ${JSON.stringify(element[i])}}`;

        elementsUndefined.push({
          schema: local,
          element: i,
          primitive: primitive,
        });
  ...
  //#endregion
  while (
    Object.keys(nodesReady).length > 0 ||
    Object.keys(newNodes).length > 0
  ) {
  ...
  for (var i in elementsUndefined) {
    ...
    if (elementsUndefined[i].primitive) {
      log +=
        " => ATTENTION THIS IS A PRIMITIVE ELEMENT, IT MAY NOT BE ACCURATE!!!";
    }
